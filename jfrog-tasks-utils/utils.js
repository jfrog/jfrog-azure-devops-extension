...
function syncRequestWithRetry(method, url, options = {}, maxRetries = 3, retryDelay = 1000) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            console.warn(`Attempt ${attempt}/${maxRetries}: Request failed for ${url} - ${err && err.message}`);
            errorToThrow = err;
        }
...
function fetchLatestCliVersion() {
    try {
        ...
        if (res.statusCode === 200) {
            ...
            console.log('Fetched ' + (releases && releases.length ? releases.length : 0) + ' JFrog CLI releases');
