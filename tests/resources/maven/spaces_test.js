...
function verifyPomFileIsQuoted() {
    ...
    assert(!resultBefore.includes('"path/with spaces/pom.xml"'), 
        'Before fix: POM file path should NOT be quoted');
    
    assert(resultAfter.includes('-f "path/with spaces/pom.xml"'), 
        'After fix: POM file path should be quoted');
