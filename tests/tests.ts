...
import conanUtilsV2 from '../tasks/JFrogConanV2/conanUtils';
...
describe('JFrog Artifactory Extension Tests', (): void => {
    ...
    describe('Conan V2 Task Tests', (): void => {
        runSyncTest(
            'Conan Custom Command',
            (): void => {
                const testDir: string = 'conanTaskV2';
                mockTask(testDir, 'conanCustomCommand');
            },
            TestUtils.isSkipTest('conan')
        );
        ...
        runSyncTest(
            ...
            (): void => {
                ...
                mockTask(testDir, 'conanUpload');
                mockTask(testDir, 'publishBuildInfo');
                getAndAssertBuild('conanTaskV2', '1');
                deleteBuild('conanTaskV2');
            },
            TestUtils.isSkipTest('conan')
        );
    });
...
function testGetCliPartialsBuildDir(): void {
    ...
    testDTO.testsBuildNames.forEach((element: string): void =>
        assertPathExists(conanUtilsV2.getCliPartialsBuildDir(element, testDTO.testBuildNumber))
    );
