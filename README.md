...
When authenticating with JFrog we offer the following authentication methods:

- Basic Authentication
    Provide username and password in the service connection with permissions to deploy.
- Token Based Authentication
    Provide access token with permissions to deploy.
- Oidc authentication
    In order to configure Oidc authentication we need to initially setup a oidc integration provider in Jfrog. See documentation on how to configure oidc [here](https://jfrog.com/help/r/jfrog-platform-administration-documentation/configure-an-oidc-integration)
    Obtaining a JFrog Access Token

To obtain a JFrog access token, follow these simple steps:

Prepare the JWT: First, you need a valid JWT (JSON Web Token), which in this case is provided by Azure DevOps (ADO). This JWT represents the identity of the requestor and will be used for authentication. Its important that you extract the information in this token and configure the identity mapping in JFrog Oidc integration provider to assert information in the provided token.

This is the format of a JWT:
...
note: the above exact values can be viewed in the system log when executing a build in azure devops with a oidc authenticated service connection
provider_name: The name of the OIDC provider, which is necessary to link the request to JFrog's authentication provider.

Use the Access Token: The obtained access token can now be used to authenticate against JFrog services, allowing you to interact with repositories, publish artifacts, and more. This is all done by associating your JFrog task to the service connection configured with oidc.
